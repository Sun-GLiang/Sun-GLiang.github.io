...
- 在中断发生时后，进入中断处理的程序是操作系统程序（被中断的程序可能是用户程序也可能是内核程序）；
    - 若被中断的是用户程序，则系统从目态转入管态，在管态下进行中断的处理
    - 若被中断的是低级中断，则仍然在管态
...
- 在内核态执行（是否需要访问特权资源【如直接操作**硬件**、修改系统表、**内核数据结构**等】，是否需要管理硬件资源和进程状态）
    ...
    - 输入输出指令（涉及中断）
    - I/O指令
    - 系统调用的**执行**（调用发生在用户态，执行发生在内核态）
        - 系统调用执行完成后，导致CPU从内核态转为用户态
        - 中断返回时，切回到用户态
        - 操作系统通过执行软中断指令陷入内核态执行系统调用，系统调用执行完成后，恢复被中断的进程或设置新进程的CPU现场，然后返回被中断进程或新进程。只有系统调用是用户进程调用内核功能，CPU从用户态切换到内核态，执行完后再返回到用户态
    - 关中断指令
    - 屏蔽中断指令
    - **设置**时钟指令
    - 停机指令
...
- Windows操作系统是融合了宏内核和微内核的操作系统
- **引导程序**
    1. 一种是位于**ROM**中的自举程序（BIOS的组成部分），用于**启动具体的设备**
    2. 一种是位于装有操作系统**硬盘**的活动分区的引导扇区中的引导程序，用于**引导操作系统**
- 计算机启动过程
...
!!! note "阻塞态"
    单处理器系统中，任何时刻都只有一个进程处于运行态 【X】</tr>
    **解：**在系统发生死锁时，有可能全部都处于阻塞态，CPU空闲

- 单处理器系统中，若同时存在10个进程，则可能
    - 9个处于就绪态，1个处于运行态
    - 10个都处于阻塞态（死锁）
    - 不可能出现10个都处于就绪态
- 进程与程序的根本区别是：静态和动态
- 分时系统中，处于就绪态的进程最多：进程都在争夺CPU的使用权，而CPU的数量是有限的
- 进程申请外设
    - 若外设是独占设备，且正在被其他进程使用，则阻塞；
    - 否则，若空闲或共享设备，则不阻塞
- 进程执行信号量的wait操作
    - 若信号量的值小于等于0，则阻塞；否则，不阻塞
...
3. PCB 可以被操作系统中的多个模块读或修改，如被调度程序、资源分配程序、中断处理程序以及监督和分析程序等读或修改。
      1. 当进程由于某种原因而暂停执行时，又须将其断点的处理机环境(由处理及中的各种寄存器组成)保存在PCB中。（包括通用寄存器、PC、PSW、用户栈指针
4. PCB中包含数据结构：进程标志信息、进程控制信息、进程资源信息、CPU现场信息（如进程标识符、进程状态、PC、寄存器、内存分配情况、打开文件表、进程优先级）
5. 全局变量只与用户代码有关
6. 互斥信号量通常存储在内核或共享内存中
7. 原子操作在管态下执行，常驻内存
8. 并发进程执行的相对速度与进程调度策略有关：因为进程调度策略决定了哪些进程可以获得处理机，以及获得处理机的时间长短，从而影响进程执行的速度和效率
9. 一个进程可以顺序执行一个或多个程序，只要在执行过程中改变其CPU状态和内存空间即可。但是一个进程不可以同时执行多个程序
10. 进程获得处理器运行是通过调度得到的
11. 引起创建进程的事件
    - 系统内核创建
        - 用户登录：登录进程
        - 高级调度发生时：即作业调度，从后备队列中选择一个作业调入内存，并创建相应的进程
        - 操作系统响应用户提出的请求：为用户创建一个子进程，用于执行用户指定的任务或程序
            - 如：打开浏览器
            - 如：用户进程请求对文件进行打印，则系统内核为其创建一个打印进程，使得打印进程和用户进程并发执行
    - 用户创建（应用请求）：由用户自己创建新进程
12. 阻塞：系统往往设置一些具有某特定功能的系统进程，每当这种进程完成任务后，便把自己阻塞起来以等待新任务到来。例如，系统中的发送进程，其主要工作是发送数据，若已有的数据已全部发送完成而又无新的发送请求，这时(发送)进程将使自己进入阻塞状态；仅当又有进程提出新的发送请求时，才将发送进程唤醒。
13. 进程通信
    - 共享存储区
    - 消息传递：也应用于微内核系统中，微内核与服务器之间的通信
    - 管道通信
14. 对于通常的进程，无论是系统进程还是用户进程，进程的创建、  撤消，以及要求由系统设备完成的I/O 操作，都是**利用系统调用**而进入内核，再由内核中的相应处理程序予以完成的。进程的切换同样是在内核的支持下实现的。因此我们说，不论什么进程，它们都是在操作系统内核的支持下运行的，是与内核紧密相关的
15. **运行中的进程结束：表示进程撤销**
16. 进程处于阻塞态是由于：①等待I/O；②互斥（等待信号或等待进入临界区）或同步（等待网络数据进入内存）引起的等待


**进程通信**

- 共享文件、消息传递、共享内存、文件系统（管道就是一种文件）、文件映射、套接字
- 进程之间不能通过全局变量进行通信：不同的进程拥有不同的代码段和数据段，全局变量是对同一进程而言的，在不同的进程中是不同的变量，没有任何关系，因此不能用于交换数据
...
1. 线程的优点：提高系统并发性、节约系统资源、便于进程通信。
      - 但是线程并不能增强进程安全性，因为线程共享进程的地址空间和资源，若一个线程出错，则可能影响整个进程的运行
2. 当一个进程只有一个线程时，线程和进程就是一样大的
3. 线程自己不拥有系统资源，只拥有一点在运行中必不可少的资源，但是它可以与同属一个进程的其他线程共享进程所拥有的全部资源，并共享进程的地址空间
4. 线程没有自己独立的地址空间，它共享其所属进程的空间
5. 线程与进程之间线程的通信可以直接通过它们共享的存储空间
6. 线程包含CPU现场，可以独立执行程序：线程的CPU现场指的是线程在运行时所需的一组寄存器的值，包括程序计数器、状态寄存器、通用寄存器、栈指针等。当线程切换时，操作系统会保存当前线程的CPU现场，并恢复下一个线程的CPU现场，以保证线程的正确执行
7. **一个线程可以创建和撤销另一个线程**
8. 线程可以访问其隶属进程的系统资源（属于同一进程的所有线程都具有相同的地址空间）
9. *不同的线程可以执行相同的程序*
10. 同一进程中的线程共享进程的地址空间
11. 各个线程都可以访问进程地址空间中的每个单元，所以一个线程可以读、写，甚至清除另一个线程的堆栈
12. **线程可以访问进程所拥有的已打开文件、定时器、信号量机构等**
13. 线程完成自己的任务后，或线程在运行中出现异常而被强制终止时，由于终止线程调用相应的函数执行终止操作。但是系统线程一旦被建立，就一直运行而不会终止。线程被终止后并不立即释放它所占有的资源，只有当进程中的其他线程执行了分离函数后，被终止线程才与资源分离，此时的资源才能被其他线程利用。被终止但尚未释放资源的线程仍可被其他线程调用，以使被终止线程重新恢复运行。为此，调用者线程须调用一条被称为“等待线程终止”的连接命令，来与该线程进行连接。如果在一个调用者线程调用“等待线程终止”的连接命令试图与指定线程相连接时，若指定线程尚未被终止，则调用连接命令的线程将会阻塞，直至指定线程被终止后才能实现它与调用者线程的连接并继续执行；若指定线程已被终止，则调用者线程不会被阻塞而是继续执行。
14. 对于系统级线程，在有的系统中为了减少创建和撤消一个线程时的开销，在撤消一个线程时，并不立即回收该线程的资源和 TCB，当以后再要创建一个新线程时，便可直接利用已被撤消但仍保持有资源和TCB 的线程作为新线程


**用户线程、内核线程**

- 一对一（有并发性）：**每个**用户级线程都映射到一个内核级线程。因此当某个线程被阻塞时，不会导致整个进程被阻塞
- 多对一
- 多对多（有并发性）
- 采用轮转调度（时间片）算法，进程中设置内核级线程和用户级线程的效果完全不同 ![](./imgs/)
- 用户级线程的调度单位是进程，跨进程的线程调度需要内核支持
- 在内核级线程中，同一进程中的线程切换，需要从用户态转到核心态进行，**系统开销大**
- CPU调度是在内核中进行的，在内核级线程中，调度是在线程一级进行的，因此内核可以同时调度同一个进程中的多个线程在多CPU上并行运行（用户级则不行）
- 用户级线程是在用户空间中实现的，不能直接利用系统调用获得内核的服务，当用户及线程要获得内核服务时，必须借助操作系统的帮助。因此用户及线程只能在用户态运行
- 内核级线程可以在内核态执行系统调用子程序，直接利用系统调用为它服务。内核级线程的程序实体可以在内核态运行
- **线程同步可能导致用户级线程切换** ![](./imgs/9.jpg)
    - 使用户级线程发生切换的事件是在用户态下进行的
    - 使内核级线程发生切换的事件是涉及到了内核态
- 用户级线程的创建和调度都是在用户态下实现的，不需要切换到内核态。操作系统无法直接调度用户级线程
- 用户级线程不能利用多处理器的并行性，因为操作系统只能看到进程
...
3. 低级调度（进程调度）
   1. 在对就绪队列进行调度时，会发生两对上下文的切换
      1. 第一次：当前进程的上下文保存到其PCB中，再加载操作系统内核中调度程序的上下文，执行内核中的调度程序，选择下一个要运行的进程
      2. 第二次：保存内核调度程序的上下文，加载被调度进程的上下文（将其CPU信息装入CPU的各个寄存器中）
...
- 设计调度算法时应该考虑
    ...
    - 资源利用率
    - 平均周转时间
    - 不需要考虑互斥性（是一种同步机制，保证多个进程访问临界资源时不会发生冲突）
- 单处理器中，进程占用处理器及占用时间长短是由进程的特点（CPU繁忙还是I/O繁忙、优先级）和调度策略决定的
- 短作业优先算法（有利于短作业）适用于I/O繁忙型：运行过程中频繁访问I/O端口，即会频繁放弃CPU，占用CPU的时间不会太久
- FCFS调度算法（有利于长作业）适用于CPU繁忙型：一旦占有CPU，就会运行很长时间
- 处于临界区的进程也可能因为中断或抢占而导致调度；若进程在临界区内请求一个需要等待的资源，如打印机，则主动放弃CPU
    - 正在访问临界资源的进程由于申请等待I/O操作而被中断时，它允许其他进程抢占处理器，但是不得进入该进程的临界区
    - 当进程进入临界区但尚未离开时就被迫进入阻塞态是可以的，只要其他进程在运行过程中不寻求进入该进程的临界区，就允许其允许，即分配CPU
- 中断向量存放在内存中的固定位置
- 进程上下文切换过程中不涉及主存和磁盘的数据交换
- 上下文切换发生在操作系统调度一个新进程到处理器上运行的时候，重要的一个上下文信息就是PC值，当前进程被打断的PC值作为寄存器上下文的一部分保存在现场信息中
- 响应比=（等待时间+执行时间）/执行时间
...
- 执行信号量的wait()操作后，**若成功获取资源，则线程状态不变；若未能获取资源，则线程进入阻塞态**
- 临界区是访问临界资源的那段代码
- 临界区是互斥共享资源，非共享数据不属于临界资源
- PV操作**不是**系统调用命令
- 执行P操作时的进程处于运行态
- 互斥锁可以用于多线程或多进程之间，但只有对它加锁的线程或进程才能解锁它。如果一个线程或进程试图解锁一个不属于它加锁的互斥锁，就会返回错误码
- 使用互斥锁时，`两个线程分别对两个不同的互斥锁先后加锁，但顺序相反`会导致死锁（循环等待了）
- 生产者-消费者问题用于解决多个进程之间的同步和互斥问题
- 进程并发，可能导致同步、互斥、死锁问题
- 需要进行互斥的操作是对临界资源的访问。不同线程对同一个进程内部的共享变量的访问才有可能需要进行互斥；不同进程的线程、代码段或变量之间不存在互斥访问的问题；同一个线程内部的局部变量也不存在互斥访问的问题
- TSL指令实现进程互斥时，不满足让全等待，即进不去临界资源的进程不会主动放弃CPU，会一直处于运行态（直至时间片用完），并没有阻塞进程存在
    - TSL指令是原子操作，不需要关中断来保证其不被打断
    - TSL指令原子性的原理：执行TSL的CPU所著内存总线，以禁止其他CPU在本指令结束之前访问内存
- 硬件方法、Peterson方法不满足让权等待
...
- 管程定义了系统中的共享数据结构资源与能对该数据结构进行操作的一组函数（P和V操作） 类似于私有类，其内部的资源访问只能通过函数来进行
    - 组成：管程名称、共享数据结构、一组函数、对共享数据结构设置初值的语句
...
- 产生死锁的必要条件
    ...
    2. 不可剥夺条件
    3. 请求与保持条件
    4. 循环等待条件：死锁必然产生循环等待，而发生循环等待则不一定死锁（仅当同类资源数为1时，才是等价命题）
...
- 对比
    ...
    4. 页式、段式、段页式都是动态重定位的
          1. 动态重定位的过程依赖于：可重定位装入程序、重定位寄存器、地址变换机构
...
- open()系统调用的参数含有文件名（文件的路径），会在**进程的用户打开文件表**中增加一个对应的表目，并返回该表目的索引号（文件描述符或文件句柄）。**系统打开文件表**只有在文件实体第一次被打开时才增加一个表目，也才会通过文件I/O将对应的索引节点从磁盘读入内存。当open调用的不同文件（文件名不同，则打开的文件实体可能相同）互为硬链接时，所打开的文件实体是一样的（共享同一个索引节点，因此在系统打开文件表中占用同一个表项）。当文件首次被打开时，系统会从磁盘读取相应的索引节点到内存中。
...
- 文件结构
    ...
    - 物理文件（取决于存储介质特性、操作系统管理方式、逻辑文件在存储介质上的组织形式）
        - 连续分配：容易导致外部碎片的发生
...
- 磁盘驱动器的最小读写单位是**扇区**；扇区是磁盘最小的物理存储单元，是磁头从磁盘中读取数据的最小单位（物理层意义，针对于磁盘驱动器）
- 簇/块是操作系统读写文件的基本单位；块/簇是操作系统与磁盘交互的最小数据单元。操作系统从硬盘读写一个块，即一次磁盘I/O（逻辑层意义，针对于操作系统）
- 块（数据块）的大小是在磁盘格式化时确定的
- 文件系统是操作系统的一部分，因此文件操作系统的最小单位是块
...
- 对于不同的设备，有不同的中断处理程序
- I/O操作完成后，驱动程序必须检查本次I/O操作中是否发生了错误
- ![](./imgs/3.jpg)
...
- 由设备独立性软件完成
    - 检查用户是否有权使用设备（设备保护）
    - 缓冲区管理（缓冲区是内存中的区域，显然不是由设备设备驱动程序完成的）
- 由设备驱动程序完成
    - 向设备寄存器写命令
- 由用户层软件完成
...
!!! success "题"
    已知磁盘的平均转速是r秒/转，平均寻道时间是T秒，每个磁道可以存储的字节数是N。现在向该磁盘读/写b字节的数据，采用随机寻道的方法，每道的所有扇区组成一个簇，其平均访问时间是<br/>
    **解：**共需要访问b/N个磁道（实际上是b/N上取整），每次访问一个磁道时，需要**寻道时间**T，**传输数据时间（读/写磁道）**r（因为一个磁道是一个簇，因此没有旋转延迟了，只有传输数据的时间，是访问整个磁道）。因而平均时间是(r+T)b/N
...
- 进程P通过执行系统调用从键盘接收一个字符的输入，即进程P执行了scanf()函数 ![](./imgs/6.jpg)
    1. 用户进程P调用I/O标准库的scanf()函数，scanf()调用系统调用封装函数read()，在这个函数中有一条陷阱指令，通过它陷入内核
    2. 内核调出read()对应的sys_read()系统调用服务进行执行
    3. 进入内核后，在设备无关层进行若干调用，最终到设备驱动层进行处理
    4. 设备驱动程序对本次I/O进行初始化操作，然后执行操作系统的进程调入程序scheduler()，由调度程序来阻塞用户进程P，并调度其他进程执行
    5. 在用户输入字符之前，CPU运行其他进程，键盘等待用户输入数据
    6. 当用户在键盘上输入字符后，外设向CPU发出相应的中断请求，CPU响应中断后，启动中断处理程序
    7. 中断处理程序对I/O中断进行简单通用处理后，唤醒具体的处理键盘输入的驱动程序
    8. 该驱动程序将字符从键盘控制器读入系统缓冲区（键盘驱动程序）
    9. 数据传输完成后，唤醒P，并将P插入就绪队列
    10. 进行中断返回，进程P也从系统调用返回
