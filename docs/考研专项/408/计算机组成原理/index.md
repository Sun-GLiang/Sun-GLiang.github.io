...
- 芯片扩展所需片数：字扩展倍数*位扩展倍数
  > 芯片扩展不可能出现位数变小的情况
- 片选信号低电平有效：表示各个信号经过**取交、取并**后，再统一进行**取反**，若为1，则有效；否则，无效 ![](./imgs/13.jpg) ![](./imgs/14.jpg)
...
- 流水线CPU是**时间并行技术（在相同的时间段内，充分利用不同的设备**。在多个功能部件在时间上相互错开
- RISC都采用流水线技术；而反过来不成立，因为大部分CISC同样采用了流水线技术
...
- I/O接口功能
    ...
    - I/O操作的控制与定时
    - 与主机和外设通信
- I/O接口即I/O控制器，其功能是接收主机发送的I/O控制信号，并实现主机和外部设备之间的信息交换
- 按照不同的数据传送格式，可将接口分为并行接口和串行接口
- I/O接口中的寄存器主要有数据缓冲寄存器、状态寄存器（前两者的内容都通过数据总线来传送的）、控制寄存器
